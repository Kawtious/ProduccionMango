...
temp <- data.frame(sonoraMango$YEAR, sonoraMango$TEMPANUAL, sonoraMango$PRECIPITACIONMEDIA)
```

### Visualizando los datos en tablas

* Tabla de producción de mangos al año en Sonora

```{r}
datatable(sonoraMango)
```

* Tabla de temperaturas y ml´s de lluvia al año en Sonora
...
Ahora veamos las cantidades de agua que necesita, es conocido que el mango consume mucha agua para que pueda dar frutos, necesitando un aproximado de 665 ml´s necesarios en ciertas candiades a travéz de los meses
...
Sin embargo podemos ver como en ningun momento podemos saciar la necesidad de agua para el mango a lo largo del año
...
plot(datos$PRECIPITACIONMEDIA, datos$DEFECTOSPRECIP, pch = 21, bg = colores, 
    xlab = "ml´s de lluvia", ylab = "Probabilidad de defectos")
legend("bottomleft", c("No defecto", "Si defecto"), pch = 21, col = c("green", 
    "red"))
```
